...
int RawClusterBuilderTemplate::InitRun(PHCompositeNode *topNode)
{
  ...
  if (m_use_RawTowerGeomv5)
  {
    towergeomnodename = "TOWERGEOM_" + detector + "v3";
  }
  else
  {
  ...
  for (; itr_geom != begin_end_geom.second; ++itr_geom)
  {
    ...
    if (m_use_RawTowerGeomv5 && m_project_tower_innersurface)
    {
      bemc->SetTowerGeometry(ix, iy, towerg->get_center_int_x(), towerg->get_center_int_y(), towerg->get_center_int_z());
    }
    else
    {
