...
void fit_dz_tanBeta()
{
  ...
  float _ep_tanBeta_emc, _em_tanBeta_emc;
  intree->SetBranchAddress("_epemc_dz",&_epemc_dz);
  intree->SetBranchAddress("_ememc_dz",&_ememc_dz);
  intree->SetBranchAddress("_ep_tanBeta_emc",&_ep_tanBeta_emc);
  intree->SetBranchAddress("_em_tanBeta_emc",&_em_tanBeta_emc);

  double Rnominal = Radius[ir];
  TH2* h2_dz_tanBeta = new TH2F("h2_dz_tanBeta",Form("#Deltaz(track-cluster) vs. tan#beta @ %.0f cm;tan#beta;#Deltaz(track-cluster) (cm)",Rnominal),50,-1.5,1.5,50,-10,10);
  ...
  for (int i=0; i<nevent; i++)
  {
    ...
    h2_dz_tanBeta->Fill(_ep_tanBeta_emc,_epemc_dz);
    h2_dz_tanBeta->Fill(_em_tanBeta_emc,_ememc_dz);
  }

  auto [slope, err_slope, intercept, err_intercept] = fit_gauss_slice(h2_dz_tanBeta, Form("figure/h2_dz_tanBeta_%d.pdf",(int)Rnominal),Rnominal,0);
  arr_slope[ir] = slope+Rnominal;
  arr_err_slope[ir] = err_slope;
  arr_intercept[ir] = intercept;
  arr_err_intercept[ir] = err_intercept;
  }
...
std::tuple<double,double,double,double> fit_gauss_slice(TH2* h, TString name, double Rnominal, int verbose=0)
{
  ...
  for (int i = 1; i <= h->GetNbinsX(); i+=(n+1))
  {
    ...
    projection->SetTitle(Form("%s , Bin %d tanBeta=%d",h->GetTitle(),i,(int)h->GetXaxis()->GetBinCenter(i)));
    ...
    double fitmin=maxBinCenter-2;
    double fitmax=maxBinCenter+2;
    ...
    if (verbose>1)
    {
      ...
      myText(0.2,0.95,kBlack,Form("%s , Bin %d tanBeta=%d",h->GetTitle(),i,(int)h->GetXaxis()->GetBinCenter(i)));
      can->Update();
  ...
  TF1 *linearFit = new TF1("linearFit", "[0]+[1]*x", -1.4, 1.4);
