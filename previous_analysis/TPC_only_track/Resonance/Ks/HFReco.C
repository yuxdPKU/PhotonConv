...
namespace HeavyFlavorReco
{
  ...
  bool useMyTrackMap = false;  //Alternative Track Map
  bool useContainer = false; // Save Container: svtxtrack and KFParticle
...
void PhotonConvKFPReco()
{
  ...
  kfparticle->use2Dmatching(); // TPC-only tracks
  kfparticle->requireBunchCrossingMatch(false); // TPC-only tracks
  if (useMyTrackMap) kfparticle->setTrackMapNodeName(trackmapName);
  if (useContainer) kfparticle->setContainerName(containerName);
  if (useContainer) kfparticle->saveDST();
  if (runTrackEff) kfparticle->setTrackMapNodeName("HFSelected_SvtxTrackMap");
  if (getTruthInfo) kfparticle->doTruthMatching();
  if (getCaloInfo) kfparticle->getCaloInfo();
  if (getDetectorInfo) kfparticle->getDetectorInfo();

  //Vertex parameters
  //kfparticle->useMbdVertex();
...
void KsReco()
{
  ...
  kfparticle->setMinimumTrackIPchi2(-1.);
  kfparticle->setMinimumTrackIP(-1.);
  kfparticle->setMaximumTrackchi2nDOF(500.);
