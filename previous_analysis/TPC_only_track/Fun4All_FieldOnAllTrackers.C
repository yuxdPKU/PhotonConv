...
void Fun4All_FieldOnAllTrackers(
    ...
    const std::string outdir = "./dst/",
    const std::string outfilename = "clusters_seeds_tracks",
    const bool convertSeeds = false)
{
  //std::string inputtpcRawHitFile = tpcdir + tpcfilename;
  std::string inputtpcRawHitFile = tpcfilename;
 ...
 TRACKING::pp_mode = true;
  ...
  TString outfile = outputRecoDir + outfilename + "_" + runnumber + "-" + segment;
  ...
  */
  ...
  TString dstoutfile = theOutfile + "_dst.root";
  std::string dststring(dstoutfile.Data());
  ...
  if (file_dst.good() && (file_dst.tellg() > 100))
  {
    ...
    string moveOutput = "mv " + dststring + " " + outdir + "/Reconstructed/" + to_string(runnumber);
