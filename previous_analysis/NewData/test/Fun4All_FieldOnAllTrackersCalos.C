...
void Fun4All_FieldOnAllTrackersCalos(
    const int nEvents = 1,
    const std::string tpcfilename = "DST_STREAMING_EVENT_run2pp_new_2024p002-00052519-00092.root",
    const std::string tpcdir = "/sphenix/lustre01/sphnxpro/physics/slurp/streaming/physics/new_2024p002/run_00052500_00052600/",
    const std::string calolist = "dst_calo_run2pp-00052519.list",
    std::string outDir = ".",
    ...
    const bool convertSeeds = false)
{
  ...
  std::pair<int, int> runseg = Fun4AllUtils::GetRunSegment(inputtpcRawHitFile);
  ...
  string outputValidateFileName = "TrackCalo_" + to_string(segment) + "_ks.root";
  ...
  auto hitsin = new Fun4AllDstInputManager("InputManager1");
  hitsin->fileopen(inputtpcRawHitFile);
  // hitsin->AddFile(inputMbd);
  se->registerInputManager(hitsin);

  auto caloin = new Fun4AllDstInputManager("InputManager2");
  caloin->AddListFile(calolist);
...
*/
  ...
  TrackOnly *trackanalysis = new TrackOnly("TrackOnly","trackanalysis.root");
  trackanalysis->setTrackPtLowCut(0);
  se->registerSubsystem(trackanalysis);
  ...
  tcm->setTrackPtLowCut(0.0);
  tcm->setEmcalELowCut(0.0);
  tcm->setnTpcClusters(20);
  tcm->setTrackQuality(500);
...
*/
  ...
  if (file_validation.good() && (file_validation.tellg() > 100))
  {
    ...
    string makeDirectoryMove = "mkdir -p " + outputRecoDirMove;
    system(makeDirectoryMove.c_str());
    string moveOutput = "mv " + outputValidateFile + " " + outDir + "/Reconstructed/" + to_string(runnumber);
    std::cout << "moveOutput: " << moveOutput << std::endl;
    system(moveOutput.c_str());
  }
