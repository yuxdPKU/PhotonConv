...
void EoP_kfp_v2(int runnumber=0)
{
  ...
  chain->Add(Form("../Reconstructed/TrackCalo_run5251*_ana.root"));
  chain->Add(Form("../Reconstructed/TrackCalo_run5252*_ana.root"));
  chain->Add(Form("../Reconstructed/TrackCalo_run5253*_ana.root"));
  chain->Add(Form("../Reconstructed/TrackCalo_run5254*_ana.root"));
  chain->Add(Form("../Reconstructed/TrackCalo_run5255*_ana.root"));
  ...
  for(int i = 0; i < nevent; i++)
  {
    ...
    for (int ican = 0; ican < _numCan; ican++)
    {
      ...
      for(unsigned int iem = 0; iem < _emcal_e->size(); iem++)
      {
        ...
        //if (isInRange(-0.05,dphi,0.05) && isInRange(-3,dz,5))
        {
