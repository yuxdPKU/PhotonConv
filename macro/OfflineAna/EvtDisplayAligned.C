...
void EvtDisplayAligned(int runnumber)
{
  gStyle->SetOptStat(0);
  ...
  canvas->SetLogy(1);
  canvas->Update();
  canvas->SaveAs(Form("figure/%d/Diff_dptdpz_run%d.pdf",runnumber,runnumber));
  ...
  canvas1->Update();
  canvas1->SaveAs(Form("figure/%d/Diff_phi_run%d.pdf",runnumber,runnumber));
  ...
  canvas2->Update();
  canvas2->SaveAs(Form("figure/%d/eOp_p_run%d.pdf",runnumber,runnumber));
  ...
  for (int index = 0; index<nevent; index++)
  {
    ...
    gr_tpc_matched->SetMarkerSize(0.5);
    ...
    can->SaveAs(Form("figure/%d/EvtDisplay_xy_run%d_event%d.pdf",runnumber,runnumber,index));
  ...
  for (int index = 0; index<nevent; index++)
  {
    ...
    can->SaveAs(Form("figure/%d/EvtDisplay_rz_run%d_event%d.pdf",runnumber,runnumber,index));
