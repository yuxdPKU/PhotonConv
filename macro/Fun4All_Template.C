...
void Fun4All_Template(vector<string> myInputLists = {"DST_TRKR_HIT_run2pp_new_2024p004-00046977-00006.root", "DST_CALO_run2pp_new_2024p004-00046977-00000.root"}, const int nEvents = 10, const bool convertSeeds = false)
{
  int verbosity = 0;
  bool IsTpcOnlyTracking = true;
  ...
  string outputJsonDir = outDir + "/inReconstruction/" + to_string(runnumber) + "/EvtDisplay/segment_" + to_string(segment) + "/" ;
  ...
  if(IsTpcOnlyTracking)
  {
    double dz_separation_0 = -0.13*2*105;
    //double dz_separation_0 = 0;
    G4TPC::tpc_drift_velocity_reco = (8.0 / 1000) * 107.0 / 105.0 * (1+ dz_separation_0 / 2. / 105.);
    //cm/ns
  ...
  float new_cemc_rad = 100.70;//(1-(-0.077))*93.5
  ...
  {
    string outputJsonDirMove = outDir + "/Reconstructed/" + to_string(runnumber) + "/EvtDisplay/segment_" + to_string(segment) + "/";
