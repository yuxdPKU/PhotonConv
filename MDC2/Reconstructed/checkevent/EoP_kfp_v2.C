...
void EoP_kfp_v2(std::string list = "runlist")
{
  int verbosity = 0;
  ...
  std::vector<std::string> filelist = readFileNameFromText(list);
  const int nfiles = filelist.size();

//for (int ifile = 0; ifile<nfiles; ifile++)
for (int ifile = 44; ifile<45; ifile++)
{
cout<<"reading "<<filelist.at(ifile)<<endl;

  TChain* chain = new TChain("tree_KFP");
  TString file = filelist.at(ifile);
  chain->Add(file);

  setBranch_kfp(chain);

  TFile* outputfile = new TFile(Form("./eop_%d_kfp_v2.root",ifile),"recreate");
  ...
  float teop_ep_px, teop_ep_py, teop_ep_pz, teop_ep_pt, teop_ep_eta, teop_ep_phi, teop_ep_pE, teop_ep_phi_corr;
  float teop_em_px, teop_em_py, teop_em_pz, teop_em_pt, teop_em_eta, teop_em_phi, teop_em_pE, teop_em_phi_corr;
  float teop_ep_radius, teop_em_radius, teop_SV_radius, teop_SV_radius_corr;
  float teop_ep_x, teop_ep_y, teop_ep_z;
  float teop_em_x, teop_em_y, teop_em_z;
...
cout<<"teop_gamma_mass = "<<teop_gamma_mass<<endl;
      ...
      teop_SV_radius = sqrt( pow((_ep_x->at(ican)+_em_x->at(ican))/2,2) + pow((_ep_y->at(ican)+_em_y->at(ican))/2,2) );
